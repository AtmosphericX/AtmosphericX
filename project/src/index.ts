...
new Cron(configurations?.internal_settings?.global_update, () => { modules.networking.fetchCacheData(); modules.atmsxparser.initSpotterUpdating(); });
new Cron(configurations?.internal_settings?.random_update, () => { modules.atmsxparser.initRandomService(); modules.atmsxpulsepoint.initRandomService();  });
new Cron(configurations?.internal_settings?.update_check,  () => { modules.utilities.getLatestUpdate(); });
modules.utilities.log({
    title: `${modules.utilities.ansi_colors.GREEN}Bootstrap${modules.utilities.ansi_colors.RESET}`,
    message: `${Object.keys(modules).length} modules activated, ${Object.keys(endpoints).length} endpoints registered, ${Object.keys(strings).length} strings loaded, and ${Object.keys(cache.internal.configurations).length} configurations cached.`
});
